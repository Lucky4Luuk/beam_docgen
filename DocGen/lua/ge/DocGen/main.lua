...
M.function_to_docs = function(name, f)
    ...
    t["Function"] = { source = info.source, lastlinedefined = info.lastlinedefined } -- , lines = info.activelines
...
M.table_to_docs = function(name, t, seen)
    ...
    for k, v in pairs(t) do
        ...
        if type(k) ~= "number" then
            ...
            if arrContains(seen, kname) then -- we want to skip anything we have already seen to avoid looping references
